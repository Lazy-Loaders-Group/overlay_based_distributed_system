...
> **Note:** Each project group consists of 5 students. You are free to pick your group members.
## 1. Goals
...
### Phase 1: Network Topology and Node Contents
...
- A new node that comes up gets connected to 2 randomly selected nodes existing in the distributed system. A Bootstrap Server (BS) is provided to facilitate this step.
...
### Phase 2: Socket-Based File Search Solution
...
### Phase 3: File Transfer with Web Services or REST API
...
2. Check with the instructor before using any high-level libraries or reusing any 3rd-party code.
3. Get approval for your design from the instructor before committing lots of time for coding.
4. Actively participate to discussions on Yammer and talk to lecturer whenever you have concerns.

### Implementation Phase

5. Develop Phase 2 of the solution.
6. Develop Phase 3 of the solution.

### Testing and Analysis Phase

7. Conduct a performance analysis using solutions for both Phase 2 and Phase 3.
8. Ensure that at least 10 nodes are in the system.
...
- Pick 3 nodes randomly and issue list of queries in the given file, one after the other (it is not essential to issue parallel queries from the same node). Each query should attempt (best effort) to find at least 1 node with the given file name, if such a node exists. You should be able to search for the entire file name as well as parts of it.
  - **Example:** If query ask for "Lord", "Lord rings", or "Lord of the rings", "lord of the rings" should be considered as a match. Consider only complete words, e.g., if you search for "Lord", file with "Lo Game" is not a match, similarly if you search "Lo", "Lord of the ring" is not a match.
- Find the number of application-level hops and latency required to resolve each query. After resolving all the queries, find the number of query messages received, forwarded, and answered by all 10+ nodes. Also find their routing table sizes and any routing related overhead/messages that may be involved (if any).

#### Step 2: Node Removal Testing

- Remove 2 random nodes form the distributed system one at a time. Before leaving, a node must inform all the nodes in its routing table that it is leaving using LEAVE message. It must also tell the BS that it is leaving (using UNREG message). We will consider only graceful departure of nodes.
- Repeat Steps 1 and 2, and collect all the statistics.

#### Step 3: Statistical Analysis

- Find min, max, average, and standard deviation of hops, latency, messages per node, and node degree. Also find per query cost and per node cost. Plot distribution (CDF) of hops, latency, messages per node, and node degree.

#### Step 4: Report and Evaluation

- Prepare a report by including your findings and critically evaluating them.
- Discuss how your solution will behave, if number of queries (Q) are much larger than number of nodes (N) (Q >> N) and vice versa (N >> Q). Comment on how to improve the query resolution while reducing messages, hops, and latency.

#### Step 5: Personal Reflection

- Write a personal reflections statement that explains your contribution to project, other group members contributions, what you learned from the project, what you like and dislike, etc. This is a private document that's not shared with other team members.
## 4. Protocol
...
BS will send the following message:
...
> **Note:** To simplify the debugging BS will actually return all the nodes known to it. However, you should use only the first 2 nodes to connect.

#### Unregister Request Message

Used to unregister from the BS:
...
> **Note:** For any message BS can't understand it will send an error of the format in Section 4.5.
...
Then issue register and unregister commands. Example:
...
Used to indicate presence of new node to other nodes that is found from BS:
...
Used to indicate this node is leaving the distributed system:
...
Used to locate a key in the network:
...
```
(Suppose we are searching for "Lord of the rings")
...
Response to query originator when a file is found:
...
```
(Suppose we are searching for string "baby")
...
- Need to demonstrate that the files can be searched with reasonable success rate
- Should work even if few random nodes are removed.
- Plan for a 15 min demo and 5 min viva.
- All group members must be present.

## 6. What to Submit

**Deadline 1:**
- Design document (must not exceed 3 pages)

**Deadline 2 and 3:**
- Your code and the makefile (if any)
- `readme.txt` file with instructions for compilation and how to run your nodes
- **Note:** Please do NOT submit executables. Submit all files as a single compressed file (.zip or .tar.gz is preferred).

**Deadline 4:**
- Report analyzing performance (must not exceed 5 pages)
- Personal reflections statement

> **Important:** Submit files to Moodle by respective deadlines.
...
| Personal Reflection Statement | 10% | Individual reflection on the project experience. |
